 ...
 **value, u64 flags) Description Add or update the value of the entry associated
 *to *key* in *map* with *value*. *flags* is one of:
 *
 * 		**BPF_NOEXIST**
 ...
 *u32 len, u64 flags) Description Store *len* bytes from address *from* into the
 *packet associated to *skb*, at *offset*. *flags* are a combination of
 ...
 *must know the former value of the header field that was modified (*from*), the
 *new value of this field (*to*), and the number of bytes (2 or 4) for this
 ...
 *void *data, u64 size) Description Write raw *data* blob into a special BPF
 *perf event held by *map* of type **BPF_MAP_TYPE_PERF_EVENT_ARRAY**. This perf
 ...
 *len) Description This helper was provided as an easy way to load data from a
 ...
 *char *optval, int optlen) Description Emulate a call to **setsockopt()** on
 ...
 *sockets. The *skops* is used as a new value for the entry associated to *key*.
 **flags* is one of:
 *
 * 		**BPF_NOEXIST**
 * 			The entry for *key* must not exist in the map.
 ...
 *char *optval, int optlen) Description Emulate a call to **getsockopt()** on
 *the socket associated to *bpf_socket*, which must be a full socket. The
 **level* at which the option resides and the name *optname* of the option must
 ...
 *policies at the socket level. If the message *msg* is allowed to pass (i.e. if
 ...
 *bpf_xfrm_state *xfrm_state, u32 size, u64 flags) Description Retrieve the XFRM
 *state (IP transform framework, see also
 ...
 **to, u32 len, u32 start_header) Description This helper is similar to
 ***bpf_skb_load_bytes**\ () in that it provides an easy way to load *len* bytes
 *from *offset* from the packet associated to *skb*, into the buffer pointed by
 **to*. The difference to **bpf_skb_load_bytes**\ () is that a fifth argument
 ...
 **map, void *key, u64 flags) Description Add an entry to, or update a sockhash
 **map* referencing sockets. The *skops* is used as a new value for the entry
 ...
 *the outermost IPv6 Segment Routing Header can be modified through this helper.
 *
 * 		A call to this helper is susceptible to change the underlaying
 * 		packet buffer. Therefore, at load time, all checks on pointers
 * 		previously done by the verifier are invalidated and must be
 ...
 **map, void *key, u64 flags) Description Select a SO_REUSEPORT sk from a
 *BPF_MAP_TYPE_REUSEPORT_ARRAY map It checks the selected sk is matching the
 *incoming request in the skb. Return 0 on success, or a negative error in case
 *of failure.
 */
#define __BPF_FUNC_MAPPER(FN)                                                  \
	FN(unspec), FN(map_lookup_elem), FN(map_update_elem),                  \
		FN(map_delete_elem), FN(probe_read), FN(ktime_get_ns),         \
		FN(trace_printk), FN(get_prandom_u32),                         \
		FN(get_smp_processor_id), FN(skb_store_bytes),                 \
		FN(l3_csum_replace), FN(l4_csum_replace), FN(tail_call),       \
		FN(clone_redirect), FN(get_current_pid_tgid),                  \
		FN(get_current_uid_gid), FN(get_current_comm),                 \
		FN(get_cgroup_classid), FN(skb_vlan_push), FN(skb_vlan_pop),   \
		FN(skb_get_tunnel_key), FN(skb_set_tunnel_key),                \
		FN(perf_event_read), FN(redirect), FN(get_route_realm),        \
		FN(perf_event_output), FN(skb_load_bytes), FN(get_stackid),    \
		FN(csum_diff), FN(skb_get_tunnel_opt), FN(skb_set_tunnel_opt), \
		FN(skb_change_proto), FN(skb_change_type),                     \
		FN(skb_under_cgroup), FN(get_hash_recalc),                     \
		FN(get_current_task), FN(probe_write_user),                    \
		FN(current_task_under_cgroup), FN(skb_change_tail),            \
		FN(skb_pull_data), FN(csum_update), FN(set_hash_invalid),      \
		FN(get_numa_node_id), FN(skb_change_head),                     \
		FN(xdp_adjust_head), FN(probe_read_str),                       \
		FN(get_socket_cookie), FN(get_socket_uid), FN(set_hash),       \
		FN(setsockopt), FN(skb_adjust_room), FN(redirect_map),         \
		FN(sk_redirect_map), FN(sock_map_update), FN(xdp_adjust_meta), \
		FN(perf_event_read_value), FN(perf_prog_read_value),           \
		FN(getsockopt), FN(override_return),                           \
		FN(sock_ops_cb_flags_set), FN(msg_redirect_map),               \
		FN(msg_apply_bytes), FN(msg_cork_bytes), FN(msg_pull_data),    \
		FN(bind), FN(xdp_adjust_tail), FN(skb_get_xfrm_state),         \
		FN(get_stack), FN(skb_load_bytes_relative), FN(fib_lookup),    \
		FN(sock_hash_update), FN(msg_redirect_hash),                   \
		FN(sk_redirect_hash), FN(lwt_push_encap),                      \
		FN(lwt_seg6_store_bytes), FN(lwt_seg6_adjust_srh),             \
		FN(lwt_seg6_action), FN(rc_repeat), FN(rc_keydown),            \
		FN(skb_cgroup_id), FN(get_current_cgroup_id),                  \
...
enum bpf_lwt_encap_mode { BPF_LWT_ENCAP_SEG6, BPF_LWT_ENCAP_SEG6_INLINE };
